...
class StockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def search(self, request):
        ...
        if not FINNHUB_API_KEY:
            return create_error_response(_("API key not configured."), errors=[_("Finnhub API key missing.")])
        ...
        try:
            ...
            if not results:
                return create_success_response(_("No stocks found for your search."), data={'results': [], 'count': 0, 'query': query})

            stocks = [{"symbol": item["symbol"], "name": item["description"]} for item in results]
            return create_success_response(_("Search results retrieved successfully."), data={'results': stocks, 'count': len(stocks), 'query': query})

        except requests.exceptions.RequestException as e:
            return create_error_response(_("Search failed."), errors=[str(e)])
        except Exception as e:
            return create_error_response(_("Internal server error."), errors=[str(e)])
