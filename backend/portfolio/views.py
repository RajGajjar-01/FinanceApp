...
class StockViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            return create_success_response(
                _("Stock retrieved successfully."),
                data=serializer.data
            )
        except NotFound:
            return create_error_response(
                _("Stock not found."),
                errors=[_("Stock does not exist")]
            )
        except Exception as error:
            return create_error_response(
                _("Failed to retrieve stock."),
                errors=[str(error)]
            )
    ...
    def search(self, request):
        try:
            ...
            if not query:
                return create_error_response(
                    _("Search query required."),
                    errors=[_("Please provide a search query")]
                )
            
            if len(query) < 2:
                return create_error_response(
                    _("Search query too short."),
    ...
    def exchanges(self, request):
        try:
            ...
            return create_error_response(
                _("Failed to retrieve exchanges."),
                errors=[str(error)]
            )
...
from backend.utils import create_success_response, create_error_response
from .models import Portfolio, Stock, PortfolioSummary
from .serializers import (
    PortfolioSerializer, PortfolioListSerializer, PortfolioCreateSerializer,
    PortfolioSummarySerializer, StockListSerializer
)
...
class PortfolioViewSet(viewsets.ModelViewSet):
    ...
    def _apply_value_filters(self, queryset, request):
        ...
        if min_value:
            try:
                ...
                ).filter(current_val__gte=Decimal(min_value))
            except (ValueError, TypeError, InvalidOperation):
                pass
        ...
        if max_value:
            try:
                queryset = queryset.annotate(
                    current_val=F('shares_owned') * F('stock__current_price')
                ).filter(current_val__lte=Decimal(max_value))
            except (ValueError, TypeError, InvalidOperation):
                pass
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            sector = request.query_params.get('sector')
            exchange = request.query_params.get('exchange')
            gain_loss_filter = request.query_params.get('gain_loss')  # 'profit', 'loss', 'breakeven'
            ...
            if gain_loss_filter == 'profit':
                queryset = queryset.annotate(
                    current_val=F('shares_owned') * F('stock__current_price')
                ).filter(current_val__gt=F('total_invested'))
            elif gain_loss_filter == 'loss':
                queryset = queryset.annotate(
                    current_val=F('shares_owned') * F('stock__current_price')
                ).filter(current_val__lt=F('total_invested'))
            elif gain_loss_filter == 'breakeven':
                queryset = queryset.annotate(
                    current_val=F('shares_owned') * F('stock__current_price')
                ).filter(current_val=F('total_invested'))
            ...
            return create_error_response(
                _("Failed to retrieve portfolio holdings."),
                errors=[str(error)]
            )
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            serializer = self.get_serializer(instance)
            return create_success_response(
                _("Portfolio holding retrieved successfully."),
                data=serializer.data
            )
        except NotFound:
            return create_error_response(
                _("Portfolio holding not found."),
                errors=[_("Holding does not exist or you don't have permission to view it")]
            )
        except Exception as error:
            return create_error_response(
                _("Failed to retrieve portfolio holding."),
                errors=[str(error)]
            )
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return create_error_response(
                _("Failed to add stock to portfolio."),
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if serializer.is_valid():
                holding = serializer.save()
                return create_success_response(
                    _("Portfolio holding updated successfully."),
                    data=serializer.data
                )
            
            return create_error_response(
                _("Failed to update portfolio holding."),
                errors=serializer.errors
            )
        except NotFound:
            return create_error_response(
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            return create_success_response(
                _("Stock removed from portfolio successfully."),
                data={}
            )
        except NotFound:
            return create_error_response(
            ...
            return create_error_response(
                _("Failed to remove stock from portfolio."),
                errors=[str(error)]
            )
    ...
    def summary(self, request):
        try:
            ...
            return create_success_response(
                _("Portfolio summary retrieved successfully."),
                data=serializer.data
            )
        except Exception as error:
            return create_error_response(
                _("Failed to retrieve portfolio summary."),
                errors=[str(error)]
            )
    ...
    def performance(self, request):
        try:
            ...
            return create_success_response(
                _("Portfolio performance retrieved successfully."),
                data=performance_data
            )
        except Exception as error:
            return create_error_response(
                _("Failed to retrieve portfolio performance."),
                errors=[str(error)]
            )
    ...
    def by_sector(self, request):
        try:
            ...
            return create_error_response(
                _("Failed to retrieve portfolio holdings by sector."),
                errors=[str(error)]
            )
    ...
    def bulk_add(self, request):
        try:
            ...
            if not holdings_data:
                return create_error_response(
                    _("No holdings data provided."),
                    errors=[_("Please provide an array of holdings to add")]
                )
            
            if len(holdings_data) > 50:  # Limit bulk operations
                return create_error_response(
                    _("Too many holdings."),
            ...
            if created_holdings:
                ...
                return create_success_response(
                    _("Bulk add completed."),
                    data=response_data
                )
            else:
                return create_error_response(
                    _("Failed to add any holdings."),
                    errors=errors
                )
                
        except Exception as error:
            return create_error_response(
                _("Failed to bulk add holdings."),
                errors=[str(error)]
            )
    ...
    def top_performers(self, request):
        try:
            ...
            sorted_holdings = sorted(
                holdings, 
                key=lambda h: h.unrealized_gain_loss_percentage, 
                reverse=True
            )[:limit]
            ...
            return create_error_response(
                _("Failed to retrieve top performers."),
                errors=[str(error)]
            )
    ...
    def worst_performers(self, request):
        try:
            ...
            # Sort by gain/loss percentage (ascending for worst)
            sorted_holdings = sorted(
                holdings, 
                key=lambda h: h.unrealized_gain_loss_percentage
            )[:limit]
            ...
            return create_error_response(
                _("Failed to retrieve worst performers."),
                errors=[str(error)]
            )
